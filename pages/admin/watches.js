...
import { deleteData, getData, postData } from '../../__lib__/helpers/HttpService';
...
export default function Home() {
    ...
    const onSubmit = data => {
        setDisable(true)
        postData('/admin/watch', data, setDisable)
        .then(res=>{
            if(res?.success)
            {  
    ...
    const handleDelete = _id =>{
        deleteData(`/admin/watch?_id=${_id}`)
        .then(res=>{
            toast.success(`${res.message}`);
            fetchVideo();
        })
                ...
                <div className="col-md-8">
...

                    {
                        !loading && <table className="table">
                            ...
                            <tbody>
                            {
                                videos && videos.reverse().map((item, index)=>{
                                    return(
                                        <tr key={index}>
                                            ...
                                            <td className="text-center text-danger">
                                                <i onClick={()=>handleDelete(item._id)} role="button" class="fas fa-trash"></i>
                                            </td>
                                        </tr>
                                    )
                                })
